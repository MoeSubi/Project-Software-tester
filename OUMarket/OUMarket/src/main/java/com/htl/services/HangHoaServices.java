...
import com.htl.oumarket.FHomeController;
...
public class HangHoaServices {
    ...
    public void ThemHangHoa(HangHoa ha) throws  SQLException{
            try(Connection conn = JDBCUtils.getConn()){
                ...
                PreparedStatement stm= conn.prepareStatement("INSERT INTO hanghoa(id,tenhanghoa,xuatxu,dongia)" + "VALUES(?,?,?,?)");
                stm.setInt(1, ha.getId());
                stm.setString(2, ha.getTenHangHoa());
                stm.setString(3, ha.getXuatXu());
                stm.setInt(4, ha.getDonGia());
                stm.executeUpdate();
                conn.commit();
            }
            catch (SQLException ex){
